...
| RSK-0012 | Risk | A legacy DB1 inventory could expose source data or credentials, and an ambiguous legacy target could damage unrelated VPS workloads. | High | Project owner | Before any VPS connection, inventory, deletion, recreation, or deployment decision. | Require DEC-0020 and an exact V0 authorisation; inventory metadata only, classify ownership, stop on ambiguity, and require a separately approved exact target list before any change. |
