...
| RSK-0003 | Risk | Source semantics, identifiers, licences, pagination, or personal-data constraints may invalidate a selected endpoint inventory or its variable roadmap. | High | Project owner | Before capture/proxy/DB1 work is proposed. | Review the endpoint inventory, source specification, and retention/publication proposal. |
...
| RSK-0008 | Risk | A planning template or candidate description could be mistaken for approval to inspect, capture, or use a source. | High | Maintainer | Before any source-related action or claim. | Keep planning documents explicitly non-operational; require DEC-0013 approval before reconnaissance and a completed DEC-0007 proposal before any capture proposal. |
...
| DEP-0001 | Dependency | A completed bounded reconnaissance assessment, followed by owner approval of the complete selected endpoint inventory and variable roadmap. | High | Project owner | Before any capture/proxy/DB1/infrastructure/application proposal. | Complete DEC-0013 within its limits, then resolve DEC-0007 before a capture/proxy/DB1 proposal. |
