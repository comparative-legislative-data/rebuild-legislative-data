...
| RSK-0012 | Risk | A legacy DB1 inventory could expose source data or credentials, and an ambiguous legacy target could damage unrelated VPS workloads. | High | Project owner | Before any VPS connection, inventory, deletion, recreation, or deployment decision. | Apply approved DEC-0020 through an exact V0 authorisation; inventory metadata only, classify ownership, stop on ambiguity, and require a separately approved exact target list before any change. |
...
| DEP-0004 | Dependency | Owner-approved VPS control plan plus host-specific inventory and exact change records, if a reset becomes necessary. | High | Project owner | Before any VPS access. | DEC-0020 is approved. Complete and obtain approval for V0 before access; retain V1/V2 evidence and obtain V3 approval before any change. |
