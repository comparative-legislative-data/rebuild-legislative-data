...
| --- | --- |
| Date (UTC) | 4 August 2026 |
| Review type | Triggered — DEC-0089 private-beta acceptance and closure |
| Reviewer role | Maintainer |
| Documents and records reviewed | DEC-0089 proposal and result; DB1 workstream narrative; GB-SCT data index; master endpoint matrix; decision register; handover; GOV-REV-0151; and the owner’s stated private-beta acceptance. |
| Active phase and authorising decision | D10 Parties collection package under owner-approved DEC-0089, restricted deployment complete. |
| Checks performed | Recorded the owner’s confirmation that the defined journey behaved as expected: hard-refresh, **DB1 catalogue**, **Parties and government roles**, then **Parties collection**. Confirmed that this accepts the three releases in the existing group, private retained-data presentation, provenance/structure/limits, and fixed pagination only. |
| Findings | The stated acceptance adds no source request, database write, service change, access expansion, or semantic claim. It does not qualify Party detail, Member party, Member party-role, other member/relationship routes, party affiliation, validity, continuity, or history, DB2, public access, download, generic query/search, raw-object access, charts, or research release. |
| Outcome | `PASS — DEC-0089 CLOSED; OWNER ACCEPTED`. |
| Affected work blocked | Party detail; every member/relationship route; Party roles changes; every other new route; DB2; semantic variables; public data access; download; generic query/search; raw-object access; charts/research release; and shared-host changes. |
...
| --- | --- |
| Date (UTC) | 5 August 2026 |
| Review type | Triggered — DEC-0106 private web-presentation deployment verification |
| Reviewer role | Maintainer |
| Documents and records reviewed | Governing baseline; handover; decision/risk registers; GOV-REV-0178; DEC-0106; existing web-presentation deployment path; remote deployment output; service/timer state; health and anonymous-access boundary results. |
| Active phase and authorising decision | Owner-authorised DEC-0106 private deployment of the contained frontend-only Database mirror directory and endpoint workspace. |
| Checks performed | Deployed commit `cbb5921` using `ops/deploy_web_presentation.sh`. The remote path passed production build, all 36 automated tests, capability checks and B1 packaging; it created a new web release and restarted only `cld-gb-sct-web.service`. Read-only post-release checks confirmed API/web services and DB1 D4A–D12 timers active, API/web health successful, and unauthenticated `/db1/gb-sct/research/catalogue` still `403`. |
...
| Outcome | `PASS — DEC-0106 PRIVATELY DEPLOYED; OWNER LIVE REVIEW PENDING`. |
...
| Findings | All 113 current registered routes have successful capture evidence and consistent manifest/object lineage. The strongest unresolved controls are absent controlled expected scope, 86 initial-only latest reconciliation states and undeclared broader Official Reports cadence. The audit also resolved a target-name ambiguity: `cld_gb_sct` is the cluster/service name while `cld_gb_sct_db1` is the database; no stale deployment database selector was found. |
...
| Owner decision required | Approve a narrow correction package for controlled expected scope/schedule records, then decide whether to proceed to stored-object integrity and live-source parity audits. |
...
| --- | --- |
| Date (UTC) | 5 August 2026 |
| Review type | Triggered — preparation of DEC-0109 expected-scope and update-control correction proposal |
| Reviewer role | Maintainer |
| Documents and records reviewed | Governing baseline; handover; registers; GOV-REV-0188; DEC-0045 master source inventory; active DB1 workstream and DEC-0108 assurance evidence. |
| Active phase and authorising scope | Proposal preparation only. No source, raw-object, VPS, database, service/timer, application or deployment action is authorised. |
| Checks performed | Separated the approved 64-form source inventory from the current 113-route operational registry; specified explicit future-capture, baseline-only, scheduled and upstream-availability states; required human- and machine-readable registers and consistency verification. |
| Findings | A documentation-only register can close the control ambiguity exposed by DEC-0108, but cannot establish byte integrity or current upstream parity. Annual-window and source-exception states need literal, non-collapsed representation. |
| Outcome | `PASS — DEC-0109 PREPARED; OWNER APPROVAL REQUIRED`. |
