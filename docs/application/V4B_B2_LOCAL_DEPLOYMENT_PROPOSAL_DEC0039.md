...
**Decision:** DEC-0039, approved 1 August 2026
...
| Mutable targets if approved | Only the named project staging/runtime/release paths and `cld-gb-sct-api.service` / `cld-gb-sct-web.service` unit files defined below |
...
The owner approved one bounded infrastructure work package that may perform
the ordered operations in this proposal only after every preceding gate passes.
It authorises the necessary SSH and existing sudo use for these
...
2. make and commit the narrow local runtime-metadata alignment in §4.2, then
   build and verify the production-aligned release only in the project’s VPS
   staging directory using the exact private Node runtime selected in §4;
...
- application-service PostgreSQL connection, project-database inspection,
  schema/migration, database write, runtime database login, password, secret,
  or environment file. The §3 digest check is the sole exception: it may make
  a local service-account read-only query of the *names only* in `16-main` and
  `16-bills`, emitting only a SHA-256 digest and never a database name;
...
| Protected PostgreSQL state | `16-main`, `16-bills`, and `16-cld_gb_sct` report their recorded active/down state as applicable; a local `postgres` service-account query produces only a SHA-256 digest of the ordered database-name set for `16-main` and `16-bills`; the project cluster remains loopback-only on `5434`. | Any protected-cluster state/digest/listener change. |
...
| Host compatibility | CPU architecture is one with an official Node `v24.18.1` Linux binary; libc/kernel and systemd are present at a version capable of the specified unit directives. A staged runtime and concrete units must pass execution/`systemd-analyze verify` before unit installation or start. | Unsupported architecture/ABI, unavailable directive, failed staged-runtime execution, missing required verification tool, or any need for a host-wide runtime. |
| Capacity and limits | The host has at least `2304 MiB` available memory and `2048 MiB` free space on the project filesystem before staging, in addition to the retained V4A limits of `CPUQuota=35%`, `MemoryMax=768M`, and `TasksMax=128` per new service. | A failed threshold, an increased limit, or impact that cannot be bounded to these services. |
...
private keys, environment values, database names/rows, or unrelated service
command lines.
...
`v24.18.1`. It is downloaded and verified directly in a new root-owned VPS
staging directory below `/srv/cld-gb-sct/staging/`; it is neither installed nor
retained on the maintainer’s Mac:

```text
https://nodejs.org/dist/v24.18.1/node-v24.18.1-linux-<x64|arm64>.tar.xz
https://nodejs.org/dist/v24.18.1/SHASUMS256.txt
https://nodejs.org/dist/v24.18.1/SHASUMS256.txt.asc
```
...
and the official `SHASUMS256.txt.asc` detached signature before extraction. A
missing verifier/key, failed signature, failed digest, or unexpected
architecture is a stop condition. It must not fall back to an unverified
download, a distribution package, `nvm`, a container, a build from source, or
`/usr/bin/node`.
...
as a production release. This package first makes and commits only the runtime
metadata alignment necessary to replace the B1 local `24.14.1` pin by
`24.18.1` (including the corresponding root lockfile metadata). It then
transfers a clean `git archive` of that commit to the VPS staging directory.
No Node runtime, dependency tree, or production release is created or retained
on the maintainer’s Mac.

Using the newly verified private VPS runtime, the staging procedure runs
`npm ci --ignore-scripts`, the complete B1 verification contract, and the
deterministic package check twice. It records the source commit, exact
Node/npm versions, lockfile digest, archive digest, manifest digest, and
five-test result. This alignment must add no dependency, route,
database/secret capability, source/network capability, research logic, or
public claim. If any capability scan, test, lockfile, or determinism check
fails, no final release or unit is installed.
...
The staging archive’s recorded SHA-256 is compared before extraction into the
final release path, which is owned by `root:cld-gb-sct` and not writable by
`cld-gb-sct`. No release path is overwritten. The service account may read the
release but cannot change code or dependencies. Before either unit file is
installed or enabled, the staged private binary must report `v24.18.1` and
`systemd-analyze verify` must accept the concrete §5 unit files held in the
same project staging directory. The VPS staging directory is removed only
after successful digest comparison, runtime/unit validation, and extraction;
it is a declared project-owned rollback target.
...
DEC-0039 authorises execution of the bounded B2 package in §§2–8 and retention
of its restricted verification result. Approval permits only the private
