...
| Concurrency | Two non-report requests at most; Committee and Plenary Official Reports run one at a time. |
...
| Time | Five-minute limit per response; six-hour limit for the whole run. A timeout becomes a visible condition and the run continues unless a hard stop applies. |
...
| Worker containment | One transient project-only worker, under the `cld-gb-sct` OS identity, with 1.5 GiB memory and 50% CPU ceilings. No public listener or service unit is created. |
...
3. **One baseline run:** execute the registry under the stated limits. A
   source condition is recorded and the next named unit continues. A hard stop
   ends the run and records the reason.
