...
function App() {
  ...
  async function loadDb1D19OfficialReports(route: (typeof d19OfficialReportsDb1Routes)[number], offset: number) { const response = await request(`${route.endpoint}?offset=${offset}&limit=20`); if (!response.ok) return; const panel = await response.json() as Db1Paged; setDb1D19OfficialReports((current) => ({ ...current, [route.key]: panel })); }
