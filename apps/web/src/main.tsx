...
function App() {
  ...
  return (
    <main className="site-shell">
      ...
      {view === "db1" && identity.authenticated && identity.data_layers_available ? <section className="catalogue-panel db1-panel" aria-labelledby="db1-heading"><p className="eyebrow">Restricted DB1 catalogue</p><h2 id="db1-heading">Fixed retained D4A baseline</h2><p className="panel-copy">This catalogue contains three retained, dated DB1 operational projections. It is not the live proxy, raw-object access, an unqualified mirror, or a canonical dataset.</p>{db1Feedback ? <p className="form-feedback" role="status">{db1Feedback}</p> : null}{db1Catalogue ? <><section className="source-disclosure"><p><strong>Fixed baseline, not a live feed:</strong> the named captures and projection records below do not change when the daily D4A reconciliation service runs. Any later projection needs its own named build and decision.</p><dl className="response-guide"><div><dt>Catalogue release</dt><dd><code>{db1Catalogue.catalogue.id}</code> · built {new Date(db1Catalogue.catalogue.built_at).toLocaleString()} · {db1Catalogue.catalogue.integrity_status}</dd></div><div><dt>Catalogue limits</dt><dd><ul>{db1Catalogue.limitations.map((limit) => <li key={limit}>{limit}</li>)}</ul></dd></div></dl></section><div className="catalogue-sections">{db1CatalogueSections.map((section) => { const panels = db1Catalogue.panels.filter((panel) => section.routes.includes(panel.source.route_id as never)); return <details className="catalogue-section" key={section.id}><summary className="catalogue-section-heading"><div><p className="eyebrow">Retained source group</p><h3>{section.title}</h3></div><div className="catalogue-section-state"><p>{panels.length} fixed projection{panels.length === 1 ? "" : "s"}</p><span>Show projections</span></div></summary><div className="catalogue-list">{panels.map((panel) => <Db1Panel key={panel.projection.build_id} panel={panel} />)}</div></details>; })}</div></> : <p className="panel-copy">Loading the declared DB1 catalogue…</p>}</section> : null}
