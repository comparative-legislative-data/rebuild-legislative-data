...
function App() {
  ...
  return (
    <main className="site-shell">
      ...
      {view === "catalogue" && identity.authenticated ? <section className="catalogue-panel" aria-labelledby="catalogue-heading"><p className="eyebrow">GB-SCT route inventory</p><h2 id="catalogue-heading">Transparent upstream access catalogue</h2><p className="panel-copy">This catalogue is not a project dataset or snapshot. The three approved actions open a live, fixed Scottish Parliament response at request time; every other route remains visibly unavailable.</p>{catalogueFeedback ? <p className="form-feedback" role="status">{catalogueFeedback}</p> : null}{catalogue ? <><p className="catalogue-summary"><strong>{catalogue.route_count} selected route forms</strong> · {catalogue.enabled_route_count} fixed private no-retention pass-through routes currently enabled.</p><div className="catalogue-list">{catalogue.routes.map((route) => { const relayed = route.availability === "RELAYED_PRIVATE_BETA"; return <article className="route-card" key={route.id}><div className="route-card-heading"><p className="route-group">{route.group} · {route.priority}</p><span className="route-state">{route.availability.replaceAll("_", " ")}</span></div><h3>{route.id}</h3><code>{route.template}</code><dl><div><dt>Operating class</dt><dd>{route.operatingClass.replaceAll("_", " ")}</dd></div><div><dt>Qualification</dt><dd>{route.qualification.replaceAll("_", " ")}</dd></div>{route.parameters.length > 0 ? <div><dt>Allowed parameters</dt><dd>{route.parameters.map((parameter) => `${parameter.name}: ${parameter.grammar}${parameter.required ? " (required)" : ""}`).join(", ")}</dd></div> : null}</dl><p>{route.limitation}</p>{relayed ? <div className="source-disclosure"><p><strong>Before you continue:</strong> this opens the raw response from the fixed Scottish Parliament path above. It is neither stored nor transformed here. Attribution is to the Scottish Parliament; no endorsement is implied. Personal-data and third-party-rights exceptions, no warranty, semantic limits, and source freshness still apply.</p><a className="secondary-button source-action" href={`/api/catalogue/gb-sct/${route.id}/source`} target="_blank" rel="noreferrer">Open source response in a new tab</a></div> : <button type="button" className="secondary-button" onClick={() => void requestRoute(route)}>Test route boundary</button>}</article>; })}</div></> : <p className="panel-copy">Loading the route registry…</p>}</section> : null}
