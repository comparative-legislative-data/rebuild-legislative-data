...
const db1CatalogueSections = [
  { id: "db1-bills", title: "Bills and formal stages", routes: ["gb-sct.bill-types.collection", "gb-sct.bill-stage-types.collection"] },
...
function App() {
  ...
  return (
    <main className="site-shell">
      ...
      {view === "db1" && identity.authenticated && identity.data_layers_available ? <section className="catalogue-panel db1-panel" aria-labelledby="db1-heading"><p className="eyebrow">Restricted DB1 catalogue</p><h2 id="db1-heading">Fixed retained baselines</h2><p className="panel-copy">These are retained, dated DB1 operational projections. They are not the live proxy, raw-object access, an unqualified mirror, or a canonical dataset.</p>{db1Feedback ? <p className="form-feedback" role="status">{db1Feedback}</p> : null}{db1Catalogue ? <><section className="source-disclosure"><p><strong>Fixed baseline, not a live feed:</strong> the named captures and projection records below do not change when the daily reconciliation services run. Any later projection needs its own named build and decision.</p><dl className="response-guide"><div><dt>Reference catalogue</dt><dd><code>{db1Catalogue.catalogue.id}</code> · built {new Date(db1Catalogue.catalogue.built_at).toLocaleString()} · {db1Catalogue.catalogue.integrity_status}</dd></div><div><dt>Catalogue limits</dt><dd><ul>{db1Catalogue.limitations.map((limit) => <li key={limit}>{limit}</li>)}</ul></dd></div></dl></section><div className="catalogue-sections">{db1CatalogueSections.map((section) => { const panels = db1Catalogue.panels.filter((panel) => section.routes.includes(panel.source.route_id as never)); return <details className="catalogue-section" key={section.id}><summary className="catalogue-section-heading"><div><p className="eyebrow">Retained source group</p><h3>{section.title}</h3></div><div className="catalogue-section-state"><p>{panels.length} fixed projection{panels.length === 1 ? "" : "s"}</p><span>Show projections</span></div></summary><div className="catalogue-list">{panels.map((panel) => <Db1Panel key={panel.projection.build_id} panel={panel} />)}</div></details>; })}{db1InstitutionalCatalogue ? <details className="catalogue-section"><summary className="catalogue-section-heading"><div><p className="eyebrow">Retained source group</p><h3>Institutional reference</h3></div><div className="catalogue-section-state"><p>{db1InstitutionalCatalogue.panels.length} fixed projections</p><span>Show projections</span></div></summary><div className="catalogue-list">{db1InstitutionalCatalogue.panels.map((panel) => <Db1Panel key={panel.projection.build_id} panel={panel} />)}</div></details> : null}</div></> : <p className="panel-copy">Loading the declared DB1 catalogue…</p>}</section> : null}
      <p className="boundary">Current boundary: private raw source relay plus, for eligible users, one fixed retained DB1 reference catalogue. No canonical dataset, chart, export, general DB1 query, or research release is available.</p>
    </main>
  );
}
