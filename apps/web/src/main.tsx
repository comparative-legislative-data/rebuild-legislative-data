...
const databaseMirrorSubjectGuides: Record<string, string> = {
  ...
  "Parties and government roles": "Source reference and relationship records about parties and government-role material. DB1 does not infer party affiliation, office holding or historical status.",
...
const databaseMirrorEndpointGuides: Record<string, string> = {
  ...
  "official reports": "The Scottish Parliament source's annual Official Report response for the specified chamber and year."
};

function databaseMirrorEndpointGuide(endpoint: string): string {
  return databaseMirrorEndpointGuides[endpoint] ?? `The Scottish Parliament source response for ${endpoint}. DB1 preserves the source fields without creating analytical variables.`;
}
...
function Db1ResearchReleaseCard({ release, records, onBrowse, loading }: { release: Db1ResearchRelease; records?: Db1ResearchRecords | undefined; onBrowse: (routeId: string, offset: number) => void; loading: boolean }) {
  ...
  const citation = `Scottish Parliament Open Data, ${release.source_url}, retrieved ${new Date(release.capture.retrieved_at).toISOString()}; retained by Comparative Legislative Data Database mirror (DB1), manifest ${release.capture.manifest_id}. Accessed ${new Date().toISOString()}.`;
...
function App() {
  ...
  if (view === "db1" && identity.authenticated && identity.data_layers_available) {
    return <main className="site-shell db1-shell"><header className="site-header"><a className="wordmark" href="/">Comparative <span>Legislative Data</span></a><p>Research infrastructure · Private beta</p></header><section className="db1-intro" aria-labelledby="page-title"><p className="breadcrumbs">Research access <span>/</span> Scottish Parliament <span>/</span> Database mirror</p><p className="eyebrow">Database mirror</p><h1 id="page-title">Scottish Parliament data mirror</h1><p>Explore dated Scottish Parliament API responses held in our Database mirror, with the original JSON, source link, provenance and access aids in one place.</p></section><p className="identity">Signed in as <strong>{identity.email}</strong>.</p><nav className="access-nav" aria-label="Access options"><span className="signed-in-badge">Private beta active</span><button type="button" onClick={() => { setView("catalogue"); setFormFeedback(undefined); void loadCatalogue(); }}>Live API catalogue</button><button type="button" onClick={() => void loadDb1Catalogue()}>Refresh mirror list</button><button type="button" onClick={() => { setView("settings"); setFormFeedback(undefined); }}>Settings</button>{identity.roles.includes("SUPERUSER") ? <button type="button" onClick={() => { setView("admin"); setFormFeedback(undefined); void loadApplications(); }}>Superuser review</button> : null}</nav><section className="catalogue-panel db1-panel" aria-labelledby="db1-heading"><div className="catalogue-heading"><div><p className="eyebrow">Scottish Parliament · Database mirror</p><h2 id="db1-heading">Find a source</h2><p className="panel-copy">Start with a research subject. It explains the type of source data it contains; each endpoint then identifies the retained responses and access options available.</p></div></div>{db1Feedback ? <p className="form-feedback" role="status">{db1Feedback}</p> : null}{db1ResearchCatalogue ? <><details className="source-disclosure db1-about"><summary>How the Database mirror differs from the live API</summary><p>The Database mirror retrieves selected Scottish Parliament API responses on documented schedules and stores each dated response in PostgreSQL. It provides access routes that the live API does not, while retaining the original JSON.</p><p>Because it is not live, a source may change between captures. Check the capture date for the retained response. The Database mirror does not change source values. Any later combined-download or alternative-format package will be clearly labelled as a Database mirror access format, not a single Scottish Parliament response.</p><ul>{db1ResearchCatalogue.limitations.map((limit) => <li key={limit}>{limit}</li>)}</ul></details><div className="catalogue-sections">{db1ResearchCatalogue.subjects.map((subject) => <details className="catalogue-section" key={subject.subject}><summary className="catalogue-section-heading"><div><p className="eyebrow">Research subject</p><h3>{subject.subject}</h3><p className="subject-description">{databaseMirrorSubjectGuides[subject.subject] ?? "Related Scottish Parliament source responses, preserved without analytical transformation."}</p></div><div className="catalogue-section-state"><p>{subject.endpoints.length} endpoint{subject.endpoints.length === 1 ? "" : "s"}</p><span>Show endpoints</span></div></summary><div className="catalogue-list endpoint-list">{subject.endpoints.map((endpoint) => { const years = endpoint.releases.flatMap((release) => release.source_year === null ? [] : [release.source_year]); const coverage = years.length ? years.length === 1 ? `Retained year ${years[0]}` : `Retained years ${Math.min(...years)}–${Math.max(...years)}` : "Retained response"; const allYears = years.length > 1; const allYearsUrl = `/api/db1/gb-sct/research/all-years?${new URLSearchParams({ subject: subject.subject, endpoint: endpoint.endpoint }).toString()}`; return <details className="route-card route-card-db1 endpoint-card" key={endpoint.endpoint}><summary><div className="route-badge"><div><p className="route-group">{coverage}</p><h3>{endpoint.endpoint}</h3><p className="endpoint-description">{databaseMirrorEndpointGuide(endpoint.endpoint)}</p><code>{endpoint.releases.length} retained release{endpoint.releases.length === 1 ? "" : "s"}</code></div><div className="route-badge-state"><span className="route-state">View releases</span><span className="route-expand-label">Show releases</span></div></div></summary><div className="route-details">{allYears ? <section className="year-index"><p><strong>All retained years</strong></p><p>This index lists compatible Database mirror releases and exceptions. It is not a combined download or one Scottish Parliament response.</p><a className="source-action official-action" href={allYearsUrl} target="_blank" rel="noreferrer">View retained year index</a></section> : null}<div className="release-list">{endpoint.releases.map((release) => <Db1ResearchReleaseCard key={release.route_id} release={release} records={db1ResearchRecords[release.route_id]} loading={db1LoadingRoute === release.route_id} onBrowse={(routeId, offset) => void loadDb1ResearchRecords(routeId, offset)} />)}</div></div></details>; })}</div></details>)}</div></> : <p className="panel-copy" role="status">Loading the Database mirror catalogue…</p>}</section><p className="boundary">Private access only. The Database mirror provides dated retained source responses and provenance. No live source request, DB2 variable, chart, generic database query or public research release is available here.</p></main>;
  }
