...
export class Db1Explorer {
  ...
  async mqaAnnualWindowD17(route: (typeof D17_MQA_ANNUAL_WINDOW_ROUTES)[number], offset: number, limit: number): Promise<Db1PagedResponse | undefined> {
    const client = await this.pool.connect();
    try { await client.query("begin read only"); const release = await client.query<{ projection_build_id: string }>("select projection_build_id from db1.mqa_annual_window_releases where id = $1 and source_route_id = $2 and integrity_status = 'PASS'", [route.releaseId, route.id]); const item = release.rows[0]; if (!item) { await client.query("commit"); return undefined; } const build = (await client.query<{ manifest_id: string; projection_name: string }>("select manifest_id, projection_name from db1.projection_builds where id = $1 and integrity_status = 'PASS'", [item.projection_build_id])).rows[0]; if (!build) throw new Error("D17 release build is unavailable"); const panel = await this.referencePanel(client, { routeId: route.id, sourcePath: route.path, manifestId: build.manifest_id, projectionName: build.projection_name }, item.projection_build_id, { offset, limit }); if (!panel) throw new Error("D17 release contract mismatch"); await client.query("commit"); return { ...panel, access_mode: "SERVER_SIDE_SELECTION", record_page: { offset, limit, total_records: panel.projection.projected_records, records: panel.records }, limitations: ["This is a fixed retained D17 annual source-window projection, not a live source response, a complete series, or an unqualified mirror.", "The year is part of the source URL. Pagination is the only current selection contract; no year selector, generic query, download, join, or DB2 variable is offered.", "Observed keys and types are structural evidence only; no question, motion, vote, bill, stage, date, or text semantics are asserted.", `The latest reconciliation state is ${panel.reconciliation_state}; it does not establish freshness, completeness, or cross-route consistency.`] }; } catch (error) { await client.query("rollback").catch(() => undefined); throw error; } finally { client.release(); }
  }
}
