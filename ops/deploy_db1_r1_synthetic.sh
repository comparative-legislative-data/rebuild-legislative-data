...
resume="${2:-}"
[[ -z "$resume" || "$resume" == "--resume" ]] || { echo "usage: $0 --from-clone [--resume]" >&2; exit 1; }
...
if [[ "$resume" == "--resume" ]]; then
  ...
  [[ -d "$project_root/raw/db1" ]]
  systemctl is-active --quiet "$unit" && { echo "R1 proof service is already active; refusing resume." >&2; exit 1; }
