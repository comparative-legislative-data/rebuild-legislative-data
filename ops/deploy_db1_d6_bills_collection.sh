...
staging="$(mktemp -d "$project_root/staging/db1-d6.XXXXXX")"
release_path=""
release_id=""
api_unit=/etc/systemd/system/cld-gb-sct-api.service
web_unit=/etc/systemd/system/cld-gb-sct-web.service
d6_unit=/etc/systemd/system/cld-gb-sct-db1-d6.service
d6_timer=/etc/systemd/system/cld-gb-sct-db1-d6.timer
d6_env=/etc/cld-gb-sct/secrets/db1-d6.env
...
rollback_before_source() {
  systemctl disable --now cld-gb-sct-db1-d6.timer 2>/dev/null || true
  systemctl stop cld-gb-sct-db1-d6.service 2>/dev/null || true
  rm -f "$d6_unit" "$d6_timer" "$d6_env"
...
"$runtime/node" "$release_path/scripts/migrate_db1_d6_bills_collection.mjs"
unset CLD_DB1_DATABASE_URL CLD_DB1_RAW_ROOT CLD_DB1_MIGRATION_ROLE

d6_password="$(openssl rand -hex 32)"
sudo -n -u postgres psql -h "$socket_directory" -p 5434 -d "$database" -v ON_ERROR_STOP=1 <<SQL
DO \$\$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'cld_gb_sct_d6_runner') THEN
    CREATE ROLE cld_gb_sct_d6_runner LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT;
  END IF;
END \$\$;
ALTER ROLE cld_gb_sct_d6_runner LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT PASSWORD '${d6_password}';
GRANT CONNECT ON DATABASE cld_gb_sct_db1 TO cld_gb_sct_d6_runner;
REVOKE ALL ON SCHEMA db1 FROM PUBLIC;
REVOKE ALL ON ALL TABLES IN SCHEMA db1 FROM PUBLIC;
GRANT USAGE ON SCHEMA db1 TO cld_gb_sct_d6_runner;
GRANT SELECT ON db1.source_routes TO cld_gb_sct_d6_runner;
GRANT SELECT, INSERT, UPDATE ON db1.capture_runs, db1.raw_objects, db1.manifest_entries, db1.reconciliation_cycles, db1.reconciliation_observations, db1.projection_builds, db1.projection_records, db1.projection_rejections, db1.bills_collection_releases TO cld_gb_sct_d6_runner;
GRANT USAGE ON SCHEMA db1 TO cld_gb_sct_db1_reader;
GRANT SELECT ON db1.bills_collection_releases, db1.reconciliation_observations TO cld_gb_sct_db1_reader;
SQL
d6_writer_proof="$(PGPASSWORD="$d6_password" psql -h 127.0.0.1 -p 5434 -U cld_gb_sct_d6_runner -d "$database" -Atqc "select has_database_privilege(current_user, current_database(), 'CONNECT') and has_schema_privilege(current_user, 'db1', 'USAGE') and has_table_privilege(current_user, 'db1.source_routes', 'SELECT') and has_table_privilege(current_user, 'db1.capture_runs', 'INSERT,UPDATE') and has_table_privilege(current_user, 'db1.bills_collection_releases', 'INSERT')")"
[[ "$d6_writer_proof" == "t" ]]

install -d -o root -g cld-gb-sct -m 0750 /etc/cld-gb-sct/secrets
umask 027
printf 'CLD_DB1_DATABASE_URL=postgresql://cld_gb_sct_d6_runner:%s@127.0.0.1:5434/%s\nCLD_DB1_RAW_ROOT=%s\n' "$d6_password" "$database" "$project_root/raw/db1" > "$d6_env"
chown root:cld-gb-sct "$d6_env"
chmod 0640 "$d6_env"

sed -e "s#releases/RELEASE_ID#releases/${release_id}#g" -e "s#CLD_RELEASE_ID=RELEASE_ID#CLD_RELEASE_ID=${release_id}#g" "$staging/source/ops/systemd/cld-gb-sct-db1-d6.service.template" > "$d6_unit"
cp "$staging/source/ops/systemd/cld-gb-sct-db1-d6.timer.template" "$d6_timer"
systemctl daemon-reload
source_started=1
systemctl start cld-gb-sct-db1-d6.service
[[ "$(systemctl show -p Result --value cld-gb-sct-db1-d6.service)" == "success" ]]

reader_proof="$(sudo -n -u postgres psql -h "$socket_directory" -p 5434 -d "$database" -Atqc "select has_table_privilege('cld_gb_sct_db1_reader', 'db1.bills_collection_releases', 'SELECT') and has_table_privilege('cld_gb_sct_db1_reader', 'db1.reconciliation_observations', 'SELECT') and not has_table_privilege('cld_gb_sct_db1_reader', 'db1.raw_objects', 'SELECT') and not has_table_privilege('cld_gb_sct_db1_reader', 'db1.projection_records', 'INSERT')")"
[[ "$reader_proof" == "t" ]]
d6_initial="$(sudo -n -u postgres psql -h "$socket_directory" -p 5434 -d "$database" -Atqc "select (select count(*) from db1.reconciliation_observations where source_route_id = 'gb-sct.bills.collection' and state = 'INITIAL') = 1 and (select count(*) from db1.bills_collection_releases where id = 'gb_sct_bills_collection_d6_v1' and integrity_status = 'PASS') = 1")"
[[ "$d6_initial" == "t" ]]
...
curl -sS --max-time 5 -o /dev/null -w '%{http_code}' http://127.0.0.1:3210/db1/gb-sct/bills/d6-v1 | grep -qx '403'

systemctl start cld-gb-sct-db1-d6.service
[[ "$(systemctl show -p Result --value cld-gb-sct-db1-d6.service)" == "success" ]]
d6_repeat="$(sudo -n -u postgres psql -h "$socket_directory" -p 5434 -d "$database" -Atqc "select (select count(*) from db1.reconciliation_observations where source_route_id = 'gb-sct.bills.collection' and state = 'INITIAL') = 1 and (select count(*) from db1.reconciliation_observations where source_route_id = 'gb-sct.bills.collection' and state = 'UNCHANGED') = 1")"
[[ "$d6_repeat" == "t" ]]
systemctl enable --now cld-gb-sct-db1-d6.timer
...
cleanup
trap - ERR
printf 'D6 Bills collection deployment passed for %s\n' "$commit"
