...
for cohort in a b c d; do
  sed -e "s#RELEASE_ID#${release_id}#g" "$source_root/ops/systemd/cld-gb-sct-db1-fullscope-${cohort}.service.template" > "/etc/systemd/system/cld-gb-sct-db1-fullscope-${cohort}.service"
done
systemctl daemon-reload
