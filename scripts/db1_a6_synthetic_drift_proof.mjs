...
try {
  ...
  const unit = await client.query("select request_method,request_locator from db1.response_unit where response_unit_key='__a6_assurance_synthetic__.drift' and is_synthetic");
  ...
  const insertResponse = async (raw) => {
    const saved = await client.query(`insert into db1.source_response
      ...
      values ('__a6_assurance_synthetic__.drift',$1,now(),now(),$2,$3,200,'{}'::jsonb,'application/json',$4::bytea,octet_length($4::bytea))
    ...
    return (await client.query("select source_response_id from db1.source_response where response_unit_key='__a6_assurance_synthetic__.drift' and body_sha256=$1", [sha256(raw)])).rows[0].source_response_id;
  };
  ...
  await client.query(`insert into db1.schema_drift_event
    ...
    values ($1,'__a6_assurance_synthetic__.drift',$2,$3,$4,$5,$6::jsonb)
